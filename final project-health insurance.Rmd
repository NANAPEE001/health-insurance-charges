---
title: "Final project-Health Insurance"
output: html_document
date:  "12-11-2024"
Author: Nana
---

# EXPLORATORY DATA ANALYSIS AND PREDICTION OF HEALTH INSURANCE CHARGES BASED ON AGE,BMI,SMOKING STATUS,REGION AND NUMBER OF CHILDREN
...
## Importing csv file
Link to dataset[here](https://www.kaggle.com/datasets/denywisnu/health-insurance-dataset/data). I have also attached a downloaded version for  your perusal 
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
#import and read csv file
insurance_df <- read.csv(file.choose())
```

## Structuring of data and summary statistics
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
correlation_matrix <- cor(insurance_df[, c("age", "bmi", "children", "charges")])
```
**no strong correlation seen among numerical variables**
...
## sample plots to visualize the poor correlation
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
## changing categorical variables to factors necessary for further analysis and machine learning
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
insurance_df %>% ggplot(aes(x=sex,fill=sex))+geom_bar(color="black")+labs(title = "Bar chart showing sex distribution",x="Sex")
```
...
print(sex_count)
```
## Pie chart to show sex distribution
...
## CHECKING DISTRIBUTION OF BMI USING BOXPLOT AND HISTOGRAM
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
insurance_df %>% ggplot(aes(x=bmi))+geom_boxplot()+labs(title = "Boxplot showing age distribution",x="BMI")
```

**outliers in BMI will not be removed as this is a health dataset where is possible to have real extreme BMIs based on health conditions **
...
## checking charges distribution using boxplot and histogram
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
insurance_df %>% ggplot(aes(x=charges))+geom_boxplot()+labs(title = "Boxplot showing age distribution",x="Charges")
```

**outliers in charges will not be removed as they might represent critical segments of the market (e.g., individuals with chronic health conditions or high-risk policies) or even the insurance coverage type**
...
## Finding Average charges by region
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
## Bar chart showing distribution of Charges based on regions
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
## Finding Average charges by sex
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
## T Test to check significance between charges for smokers and non smokers 

```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
# Compare charges between smokers and non-smokers
t.test(charges ~ smoker, data = insurance_df) 
```
**Based on the results,the  test is significant. null hypothesis will be rejected and conclude there is a difference in mean of charges between smokers and non smokers**


## T Test to check significance between charges for sex
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
```
**Based on the results,the  test is significant. null hypothesis will be rejected and conclude there is a difference in mean of charges between male and females**
...
```
**p value of 0.03 is less than alpha of 0.05 so the test is significant and a Tukey's test is needed**
...
print(tukey_region) 
```
**based on the results there is a sig difference between southwest and southeast since p value of 0.04 is less than alpha of 0.05**




## getting train and test data for machine learning
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
...
summary(insurance_linear_model) 
```

**the model is significant  and 76.08% of the dependent variable is explained by the model**

**Also at significance level of 0.05,the features (age,bmi,children,smokeyes,regionsouthwest) are significant and affect the dependent variable charges**
```{r,warning=FALSE,message=FALSE,echo=TRUE,eval=TRUE}
#predictions
...
```
**70.4% of the dependent variable is explained by this model **
...
print(rsq_rf)
```
**82.58% of the dependent variable charges is explained by this model**
...
```
**79.71% of the variation in the dependent variable charges is explained by this model **



**Based on the results above the random forest regressor model seems relatively to perform better than the linear and decision tree regressor model.**
